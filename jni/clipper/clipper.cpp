  ...
  public:
    ...
    Int128 operator * (const Int128 &rhs) const
    {
      if ( !(hi == 0 || hi == -1) || !(rhs.hi == 0 || rhs.hi == -1))
          assert2(false, "Int128 operator*: overflow error");
    ...
    Int128 operator/ (const Int128 &rhs) const
    {
      if (rhs.lo == 0 && rhs.hi == 0)
          assert2(false, "Int128 operator/: divide by zero");
...
bool FullRangeNeeded(const Polygon &pts)
{
  ...
  for (Polygon::size_type i = 0; i <  pts.size(); ++i)
  {
    if (Abs(pts[i].X) > hiRange || Abs(pts[i].Y) > hiRange)
        {assert2(false, "Coordinate exceeds range bounds.")}
...
void Clipper::FixHoleLinkage(OutRec *outRec)
{
  ...
  assert2(outRec != tmp, "HoleLinkage error");
...
void Clipper::InsertLocalMinimaIntoAEL( const long64 botY)
{
  while(  m_CurrentLM  && ( m_CurrentLM->Y == botY ) )
  {
    ...
    if( lb->nextInAEL != rb )
    {
      ...
      while( e != rb )
      {
        assert2(e, "InsertLocalMinimaIntoAEL: missing rightbound!");
  ...
  while( e )
  {
      ...
      ((dir == dRightToLeft) && (e->xcurr >= horzLeft)))
    {
      ...
      if( e == eMaxPair )
      {
        ...
        assert2(eMaxPair->outIdx < 0, "ProcessHorizontal error");
        return;
      }
...
{
  assert2(e->nextInLML, "UpdateEdgeIntoAEL: invalid call");
  TEdge* AelPrev = e->prevInAEL;
  TEdge* AelNext = e->nextInAEL;
...
void Clipper::DoMaxima(TEdge *e, long64 topY)
{
  ...
  else assert2(false, "DoMaxima error");
}
//------------------------------------------------------------------------------
